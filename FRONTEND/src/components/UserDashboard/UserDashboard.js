...
const CoachDashboard = () => {
  const [user, setUser] = useState(null); // State to hold coach details
  const [status, setStatus] = useState('pending'); // Status for user's profile acceptance
  const navigate = useNavigate();
    ...
    const fetchCoachData = async () => {
      try {
        ...
        if (userId && token) {
          ...
          setUser(response.data); // Set coach data from API response
          setStatus(response.data.status); // Set status from API response if available
        } else {
          navigate('/loginuser'); // If no ID or token, redirect to login
        }
      } catch (error) {
    ...
    <div className="dashboard-container">
      ...
      <main className="main-content">
        ...
        <section className="status-section">
          {/* Display status message based on acceptance status */}
          {status === 'pending' && <p>Your profile is under review by the coach.</p>}
          {status === 'accepted' && <p>🎉 Congratulations! Your profile has been accepted by the coach.</p>}
          {status === 'rejected' && <p>Unfortunately, your profile was not accepted. Please try again.</p>}
        </section>
        <section className="stats-section">
